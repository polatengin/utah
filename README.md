...
Utah supports typed variables, interfaces and records, schema validation for parsed JSON/YAML, higher-order array helpers, and collection types such as `map`, `dictionary`, and `set`.
...
- `process.isRunning()` - Check if a process with the given PID is currently running
...
- [x] `process.kill()` - Kill a running process by ID or name
