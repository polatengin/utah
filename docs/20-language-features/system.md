...
Utah can install cron jobs directly from your scripts.

### scheduler.cron()

Registers a cron job that runs on the specified schedule. The job body is written as a lambda.
...
Each `scheduler.cron()` call generates bash that:

1. Creates a `~/.utah/cron/` directory for job scripts
2. Writes the lambda body into a standalone script
3. Makes it executable
4. Checks for duplicate cron entries before installing
5. Registers the job with `crontab`
