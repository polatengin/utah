...
if [ $(eval "echo \${${connection}[connected]}") ]; then
  ...
  downloadSuccess=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/remote/file.txt" "/local/file.txt";   download_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/remote/file.txt" "/local/file.txt";   download_result=$?; fi; if [ $download_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  if [ "${downloadSuccess}" = "true" ]; then
    echo "File downloaded successfully"
  else
    echo "Failed to download file"
  fi
  configDownload=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/etc/app/config.yml" "/tmp/downloaded_config.yml";   download_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/etc/app/config.yml" "/tmp/downloaded_config.yml";   download_result=$?; fi; if [ $download_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  if [ "${configDownload}" = "true" ]; then
    echo "Configuration file downloaded"
  fi
  logDownload=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/var/log/app.log" "/local/logs/app.log";   download_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/var/log/app.log" "/local/logs/app.log";   download_result=$?; fi; if [ $download_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  scriptDownload=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/opt/scripts/deploy.sh" "/local/scripts/deploy.sh";   download_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/opt/scripts/deploy.sh" "/local/scripts/deploy.sh";   download_result=$?; fi; if [ $download_result -eq 0 ]; then echo "true"; else echo "false"; fi })
