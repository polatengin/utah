...
if [ $(eval "echo \${${connection}[connected]}") ]; then
  ...
  uploadSuccess=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/test_file.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_test_file.txt";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/test_file.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_test_file.txt";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  if [ "${uploadSuccess}" = "true" ]; then
    echo "Basic file upload successful"
    remoteContent=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   ssh_cmd="ssh -S $(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     ssh_cmd="$ssh_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     ssh_cmd="sshpass -e $ssh_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $ssh_cmd "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")" -p "$(eval "echo \${${connection}[port]}")" "cat /tmp/remote_test_file.txt"; else;   # One-time connection;   ssh_cmd="ssh";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     ssh_cmd="$ssh_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     ssh_cmd="sshpass -e $ssh_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $ssh_cmd "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")" -p "$(eval "echo \${${connection}[port]}")" "cat /tmp/remote_test_file.txt"; fi })
    echo "Remote file content: ${remoteContent}"
  else
    echo "Basic file upload failed"
  fi
  echo "Binary content with special chars: \n\t!@#$%^&*()" > "/tmp/binary_test.dat"
  binaryUpload=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/binary_test.dat" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_binary.dat";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/binary_test.dat" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_binary.dat";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  ...
  scriptUpload=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/test_script.sh" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/scripts/test_script.sh";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/test_script.sh" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/scripts/test_script.sh";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  echo "Script upload result: ${scriptUpload}"
  configContent="# Configuration file\nserver_name=test_server\nport=8080\ndebug=true\n"
  echo ${configContent} > "/tmp/app.conf"
  configUpload=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/app.conf" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/etc/app/app.conf";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/app.conf" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/etc/app/app.conf";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  ...
  upload1=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/file1.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_file1.txt";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/file1.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_file1.txt";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  upload2=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/file2.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_file2.txt";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/file2.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_file2.txt";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  upload3=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/file3.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_file3.txt";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/file3.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_file3.txt";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
  ...
  largeUpload=$({ conn_var=${connection}; if [ "$(eval "echo \${${connection}[async]}")" = "true" ]; then;   # Use control master for async connection;   scp_cmd="scp -o ControlPath=$(eval "echo \${${connection}[socket]}")";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/large_file.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_large_file.txt";   upload_result=$?; else;   # One-time connection;   scp_cmd="scp";   if [ "$(eval "echo \${${connection}[authMethod]}")" = "key" ]; then;     scp_cmd="$scp_cmd -i $(eval "echo \${${connection}[keyPath]}")";   elif [ "$(eval "echo \${${connection}[authMethod]}")" = "password" ]; then;     scp_cmd="sshpass -e $scp_cmd";   fi;   SSHPASS="$(eval "echo \${${connection}[password]}")" $scp_cmd -P "$(eval "echo \${${connection}[port]}")" "/tmp/large_file.txt" "$(eval "echo \${${connection}[username]}")@$(eval "echo \${${connection}[host]}")":"/tmp/remote_large_file.txt";   upload_result=$?; fi; if [ $upload_result -eq 0 ]; then echo "true"; else echo "false"; fi })
