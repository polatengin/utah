...
minVal=10
maxVal=20
_utah_random_min_4=${minVal}
_utah_random_max_4=${maxVal}
if [ $_utah_random_min_4 -gt $_utah_random_max_4 ]; then
  echo "Error: min value ($_utah_random_min_4) cannot be greater than max value ($_utah_random_max_4) in utility.random()" >&2
  exit 100
fi
randomVar=$((RANDOM * (_utah_random_max_4 - _utah_random_min_4 + 1) / 32768 + _utah_random_min_4))
echo "Random with vars: ${randomVar}"
_utah_random_min_5=150
_utah_random_max_5=50
if [ $_utah_random_min_5 -gt $_utah_random_max_5 ]; then
  echo "Error: min value ($_utah_random_min_5) cannot be greater than max value ($_utah_random_max_5) in utility.random()" >&2
  exit 100
fi
invalid=$((RANDOM * (_utah_random_max_5 - _utah_random_min_5 + 1) / 32768 + _utah_random_min_5))
